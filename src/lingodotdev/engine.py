...
class LingoDotDevEngine:
    ...
    def _localize_raw(
        ...
        progress_callback: Optional[Callable[[int, Dict[str, str], Dict[str, str]], None]] = None
    ) -> Dict[str, str]:
        """
    ...
    def _extract_payload_chunks(self, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for key, value in payload.items():
            ...
            if (current_chunk_size > self.config.ideal_batch_item_size or
                current_chunk_item_count >= self.config.batch_size or
    ...
    def localize_text(
        ...
        def wrapped_progress_callback(progress: int, source_chunk: Dict[str, str], processed_chunk: Dict[str, str]):
            if progress_callback:
    ...
    def localize_chat(
        ...
        for message in chat:
            if 'name' not in message or 'text' not in message:
                raise ValueError("Each chat message must have 'name' and 'text' properties")
    ...
    def recognize_locale(self, text: str) -> str:
        ...
        try:
            ...
            json_response = response.json()
            return json_response.get('locale', '')
