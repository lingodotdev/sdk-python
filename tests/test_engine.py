...
class TestLingoDotDevEngine:
    ...
    def test_count_words_in_record_dict(self):
        ...
        assert self.engine._count_words_in_record({"key1": "hello world", "key2": "test"}) == 3
        assert self.engine._count_words_in_record({}) == 0
        assert self.engine._count_words_in_record({"key1": {"nested": "hello world"}}) == 2
    ...
    def test_batch_localize_text_missing_target_locales(self):
        ...
        with pytest.raises(ValueError, match="target_locales is required"):
            self.engine.batch_localize_text(
                "hello",
                {"source_locale": "en"}
            )
    ...
    def test_localize_chat_invalid_format(self):
        ...
        with pytest.raises(ValueError, match="Each chat message must have 'name' and 'text' properties"):
    ...
    def test_whoami_success(self, mock_post):
        ...
        mock_response = Mock()
        mock_response.ok = True
        mock_response.json.return_value = {"email": "test@example.com", "id": "user_123"}
        mock_post.return_value = mock_response
